...
|---|---|---|---|---|
| 1 | **News headlines** | **RSS feeds** (primary) + Finnhub (fallback) | Free, unlimited | **Primary:** Google News, Yahoo Finance RSS, Seeking Alpha (ticker news), Federal Reserve + CNBC (macro), SEC 8-K + 10-Q (filings). **Fallback:** Finnhub `/company-news` for sentiment enrichment if API key available. `RssPrimaryDataSource` tries RSS first, falls back gracefully if RSS fails or returns no articles. See [STATUS.md](STATUS.md#86--rss-feed-news-ingestion-free-unlimited) for details. |
