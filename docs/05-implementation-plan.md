...
3. `[senior]` `shared/src/settings.ts`: the complete zod `SettingsSchema` +
   ...
   llm (Jedai), schedule, risk, paper. Include the `minIntervalHours` cron
...
11. `[senior]` `llm/authTokenProvider.ts` (copy verbatim from lexchat) +
    `llm/jedaiChatModel.ts` (adapt to take an explicit config object with
    env fallback — preserve the custom `fetch`, 401-retry, Disney headers,
    and the `modelKwargs` penalty/top_p stripping). Add `routes/llm.ts` →
    `POST /api/llm/test` doing a minimal completion and surfacing the
    gateway error body on failure.
...
validated against a live quote, enter Jedai credentials and get a green
