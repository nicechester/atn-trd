 ...
 */
export function resolveExecutionOrder(selectedJobIds) {
    // Validate all jobs exist
    const missing = selectedJobIds.filter(id => !JOB_REGISTRY[id]);
    if (missing.length > 0) {
    ...
    function expandDependencies(jobId, expanded) {
    ...
    function visit(jobId) {
        ...
        visited.add(jobId);
        const job_final = JOB_REGISTRY[jobId];
        if (job_final) {
    ...
    const sortedByCanonical = Array.from(allJobsNeeded).sort((a, b) => canonicalOrder.indexOf(a) - canonicalOrder.indexOf(b));
